...
def save_checkpoint(
    state, is_best, checkpoint_dir="checkpoints", filename="checkpoint.pth.tar"
):
...
def log_training_info(
    log_path, epoch, train_loss, val_loss=None, lr=None, is_checkpoint=False
):
    logs = []
...
for epoch in range(start_epoch, num_epochs):
    ...
    print(
        f"Epoch [{epoch + 1}/{num_epochs}], Train Loss: {avg_train_loss:.6f}, Val Loss: {val_loss:.6f}, R2: {val_r2:.4f}, LR: {current_lr:.2e}"
    )

    # 记录日志
    log_entry = log_training_info(
        log_file, epoch + 1, avg_train_loss, val_loss, current_lr, is_checkpoint=False
    )
